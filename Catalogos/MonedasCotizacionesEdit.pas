...
uses
  ...
  cxTextEdit, cxDBEdit, Vcl.DBCtrls, Vcl.StdCtrls, cxDropDownEdit, cxCalendar,
  cxMaskEdit, cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox,
  dxSkinscxPCPainter, cxPCdxBarPopupMenu, cxScrollBox, cxPC;

type
  TfrmMonedasCotizacionesEdit = class(T_frmStandarGFormEdit)
    Label1: TLabel;
    Label2: TLabel;
...
uses MonedasCotizacionesDM, MonedasCotizacionesForm;

procedure TfrmMonedasCotizacionesEdit.FormCreate(Sender: TObject);
begin
  inherited;
  gFormGrid := TfrmMonedasCotizaciones.Create(Self);
  TfrmMonedasCotizaciones(gFormGrid).CerrarGrid := actCloseGrid;
end;
